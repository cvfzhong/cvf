...
#include"IPA\mean_filter.h"
#include"BFC\autores.h"
#include"IFF\opencv.h"
...
class filter_np_buffer
{
	...
	{
		config m_cfg;
		AutoArrayPtr<_NValT> m_np;
	public:
		np_item()
		{}

		np_item(const config &cfg, AutoArrayPtr<_NValT> np)
			:m_cfg(cfg),m_np(np)
		{
		}
	};
...
public:
	const _NValT* get(int width, int height, int kszx, int kszy, int borderLTRB[])
	{
		...
		m_list.push_back(np_item(cfg,  AutoArrayPtr<_NValT>(np) ));
...
inline void mean_filter(_IntegralImageT &sum_filter, _NPBufferT &np_buffer, _DValT *dest, int dstride, _CNT cn, int kszx, int kszy, int x=0, int y=0, int width=-1, int height=-1)
{
	...
	typedef typename pval_accum_type<_DValT>::type _SumValT;
	...
	for_each_pixel_3(sum_buf,width,height,width*cn,cn,np+y*sum_filter.m_width+x,sum_filter.m_width,1,dest,dstride,cn,iop_div_p<_CNT>(cn));
...
{
	typedef typename pval_accum_type<_SValT>::type _SumValT;
	_SumValT  *sum_buf=new _SumValT[width*cn*height];
	...
	for_each_pixel_3(sum_buf,width,height,width*cn,cn,np,width,1,dest,dstride,cn,iop_div_p<_CNT>(cn));
...
{
	typedef typename default_pixel_traits<_SValT>::accum_type _SumValT;
...
_IP_API void mean_filter(const uchar *src, int width, int height, int istep, int cn, uchar *dest, int dstride, int *np, int nstride, int kszx, int kszy, int borderLTRB[])
{
	if(!np && !borderLTRB)
...
_IP_API void mean_filter(const uchar *src, int width, int height, int istep, int cn, int *dest, int dstride, int *np, int nstride, int kszx, int kszy, int borderLTRB[])
{
	/*if(!np && !borderLTRB)
		mean_filter(src,width,height,istep,cn,dest,dstride,kszx,kszy);
	else*/
...
_IP_API void mean_filter(const int *src, int width, int height, int istep, int cn, int *dest, int dstride, int *np, int nstride, int kszx, int kszy, int borderLTRB[])
{
	if(!np && !borderLTRB)
		mean_filter(src,width,height,istep,cn,dest,dstride,kszx,kszy);
	else
...
_IP_API void mean_filter(const uchar *src, int width, int height, int istep, int cn, uchar *dest, int dstride, int kszx, int kszy)
{
	ipfi_mean_filter_cx(src,width,height,istep,cn,dest,dstride,kszx,kszy);
}

_IP_API void mean_filter(const int *src, int width, int height, int istep, int cn, int *dest, int dstride, int kszx, int kszy)
{
	ipfi_mean_filter_cx(src,width,height,istep,cn,dest,dstride,kszx,kszy);
}
...
_IP_API const int *get_3x3_fixed_gaussian_table(int w0, int w1)
{
	static fgt_3x3 tab;
